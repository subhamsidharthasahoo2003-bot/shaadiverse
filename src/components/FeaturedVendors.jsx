...
const allVendors = [
  ...
  {
    id: 6,
    name: 'Sabyasachi Party & Couture Wear',
    category: 'Formal & Party Wear',
    location: 'Mumbai',
    rating: 4.9,
    reviews: 210,
  ...
  {
    id: 8,
    name: 'Horizon Corporate Planners',
    category: 'Event Planners',
    location: 'Delhi NCR',
    rating: 4.9,
    reviews: 114,
...
const FeaturedVendors = ({ selectedCategory, selectedLocation, wishlist, toggleWishlist, onSelectCategory, onSelectLocation }) => {
  ...
  const tabs = ['All', 'Event Venues', 'Photographers', 'Makeup Artists', 'Event Planners', 'Decorators'];
  ...
  return (
    <section id="vendors-section" className="py-24 bg-[#FAF9F6] border-t border-event-pink/15">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
          ...
          <p className="text-stone-500 font-light max-w-md mt-4 md:mt-0 text-sm md:text-base leading-relaxed">
            Hand-selected leading profiles with exceptional feedback, proven trust badges, and special EventVerse rates.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-10 pb-4 border-b border-stone-200">
          ...
          {(selectedCategory || selectedLocation || activeTab !== 'All') && (
            <button
              ...
              className="px-4 py-2.5 rounded-full text-xs font-bold text-event-pink bg-event-pink/10 hover:bg-event-pink/20 border border-event-pink/20 transition-colors ml-auto cursor-pointer"
            >
              Clear Filters
            </button>
          )}
        ...
        {selectedCategory || selectedLocation ? (
          <div className="mb-6 bg-event-pink/5 border border-event-pink/15 rounded-xl p-4 flex items-center space-x-2 text-stone-800 text-sm">
            <span>Showing results for: </span>
            {selectedCategory && (
              <span className="bg-event-pink/10 text-event-pink font-semibold px-2.5 py-1 rounded-md text-xs">
              ...
              {filteredVendors.map((vendor) => {
                ...
                return (
                  <motion.div
                    ...
                    className="group bg-white rounded-3xl overflow-hidden border border-stone-200/50 hover:border-event-pink/40 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col"
                  >
                    {/* Image Header */}
                    <div className="relative h-64 overflow-hidden">
                      ...
                      {/* Vendor Category Tag */}
                      <span className="absolute bottom-4 left-4 bg-white/95 text-event-pink text-[10px] font-extrabold uppercase tracking-widest px-3 py-1 rounded-full shadow-sm">
                        {vendor.category}
                      </span>
                    </div>
                    ...
                    <div className="p-6 flex-grow flex flex-col justify-between">
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center space-x-1">
                            <FiMapPin className="text-event-pink w-4 h-4" />
                        ...
                        <div className="flex items-center space-x-2 mb-3">
                          <h3 className="font-serif text-xl font-bold text-stone-900 group-hover:text-event-pink transition-colors duration-300">
                            {vendor.name}
                          </h3>
                          {vendor.verified && (
                            <span
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-stone-100 mt-4">
                        <div>
                          ...
                          <p className="text-event-pink font-bold text-base">{vendor.price}</p>
                        </div>
                        <button className="px-4 py-2 bg-stone-900 group-hover:bg-event-pink text-white font-bold text-xs rounded-xl hover:brightness-110 transition-colors duration-300 cursor-pointer">
                          View Deals
                        </button>
                      </div>
                    </div>
