...
const WhyChooseUs = () => {
  ...
  const [checklist, setChecklist] = useState([
    ...
    { id: 2, task: 'Hire Wedding Photographer & Videographer', completed: true },
    { id: 3, task: 'Select Bridal Lehenga & Groom Sherwani', completed: false },
    { id: 4, task: 'Hire Makeup Artist & Hairstylist', completed: false },
    { id: 5, task: 'Finalize Catering Menu & Tastings', completed: false },
    { id: 6, task: 'Send Digital Wedding Invitations', completed: false },
  ]);
  ...
  const features = [
    ...
    },
    {
      ...
      desc: 'Stay organized with a custom month-by-month wedding checklist tailored to your date. Click to view list.',
      color: 'from-rose-gold/20 to-rose-gold/10 hover:border-rose-gold/60',
    ...
    },
    {
      ...
      desc: 'Save hundreds of bridal makeup close-ups, reception lighting setups, and mandap designs directly to a shared mood board.',
    ...
    },
    {
      ...
      color: 'from-gold-accent/20 to-gold-accent/10 hover:border-gold-accent/50',
  ...
  return (
    <section className="py-24 bg-stone-50 border-t border-rose-gold/15 relative">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-stone-600 font-light text-sm md:text-base leading-relaxed">
            We simplify Indian wedding planning with interactive smart tools, checked lists, and certified service marketplaces.
          </p>
        </div>
          ...
          {features.map((feat, idx) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="font-serif text-xl font-bold text-stone-900 group-hover:text-burgundy transition-colors duration-300 mb-3 flex items-center">
                  <span>{feat.title}</span>
                  {feat.interactive && (
                    <span className="ml-2 bg-[#7B2D26]/10 text-[#7B2D26] text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                      Interactive
                    </span>
                  )}
                </h3>
                ...
                {feat.interactive && (
                  <span className="text-xs font-bold text-burgundy border-b border-burgundy/30 pb-0.5 group-hover:border-burgundy transition-all mt-auto uppercase tracking-wider">
        ...
        {isModalOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-6 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 w-full max-w-lg bg-[#FAF9F6] rounded-3xl shadow-2xl p-6 overflow-y-auto max-h-[85vh] z-[60] border border-rose-gold/30 text-stone-800"
            >
              {/* Header */}
              <div className="flex items-center justify-between border-b border-rose-gold/20 pb-4 mb-6">
                <div className="flex items-center space-x-2">
                  <FiSliders className="text-burgundy w-5 h-5" />
                  <h3 className="font-serif text-xl font-bold text-stone-900">
                    {modalType === 'budget' ? 'Interactive Budget Allocator' : 'Your Wedding Checklist'}
                  </h3>
                </div>
                <button
                  onClick={() => setIsModalOpen(false)}
              ...
              {modalType === 'budget' ? (
                <div>
                  <p className="text-stone-600 text-xs mb-6">
                    Adjust the slider to simulate budget allocations. Our breakdown is calculated based on statistical wedding averages.
                  </p>
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-stone-200/60 mb-6 shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-burgundy font-extrabold text-lg">{formatCurrency(budgetVal)}</span>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between bg-stone-100/50 p-3 rounded-xl">
                      ...
                      <span className="text-xs text-burgundy font-bold">{formatCurrency(venueBudget)}</span>
                    </div>
                    <div className="flex items-center justify-between bg-stone-100/50 p-3 rounded-xl">
                      <span className="text-xs text-stone-600 font-medium">Food & Catering (25%)</span>
                      <span className="text-xs text-burgundy font-bold">{formatCurrency(cateringBudget)}</span>
                    </div>
                    <div className="flex items-center justify-between bg-stone-100/50 p-3 rounded-xl">
                      <span className="text-xs text-stone-600 font-medium">Decor & Theme Setups (10%)</span>
                      <span className="text-xs text-burgundy font-bold">{formatCurrency(decorBudget)}</span>
                    </div>
                    <div className="flex items-center justify-between bg-stone-100/50 p-3 rounded-xl">
                      <span className="text-xs text-stone-600 font-medium">Photography & Cinematic Film (10%)</span>
                      <span className="text-xs text-burgundy font-bold">{formatCurrency(photoBudget)}</span>
                    </div>
                    <div className="flex items-center justify-between bg-stone-100/50 p-3 rounded-xl">
                      <span className="text-xs text-stone-600 font-medium">Bridal/Groom Wear (5%)</span>
                      <span className="text-xs text-burgundy font-bold">{formatCurrency(makeupBudget)}</span>
                    </div>
                    <div className="flex items-center justify-between bg-stone-100/50 p-3 rounded-xl">
                      <span className="text-xs text-stone-600 font-medium">Planner Services (5%)</span>
                      <span className="text-xs text-burgundy font-bold">{formatCurrency(plannerBudget)}</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full mt-6 bg-burgundy hover:bg-stone-900 text-white font-semibold py-3 rounded-xl text-xs uppercase tracking-wider transition-colors cursor-pointer"
                  >
                ...
                <div>
                  ...
                  <div className="mt-6 flex items-center justify-between text-xs text-stone-500 font-semibold bg-rose-gold/15 p-4 rounded-xl border border-rose-gold/20">
